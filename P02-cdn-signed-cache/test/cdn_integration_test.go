...
// Teste de integração do caminho completo: cliente, CDN, validador e origem.
//
...
func TestMain(m *testing.M) {
	...
	code := 1
	func() {
		// O ambiente é derrubado com os volumes: cache e log da CDN são estado
...
func esperarSaude(ctx context.Context) error {
	alvos := []string{cdnURL + "/healthz", tokendURL + "/healthz", originAPI + "/admin/counters"}
...
func TestCacheQuenteNaoDispensaAutorizacao(t *testing.T) {
	...
	status, _, _ := get(t, cdnURL+path) // agora sem token nenhum
...
func TestVetoresNegativosNoCaminhoCompleto(t *testing.T) {
	...
	casos := []struct {
		...
		{"sem token", cdnURL + path, "", http.StatusForbidden},
		{"assinatura alterada", strings.Replace(url, "sig=", "sig=ff", 1), "", http.StatusForbidden},
		{"token de outro objeto", cdnURL + objeto("outro") + "?" + queryDe(url), "", http.StatusForbidden},
	}
...
func TestHostDesconhecidoNaoEhAtendido(t *testing.T) {
	req, err := http.NewRequest(http.MethodGet, cdnURL+"/healthz", nil)
	if err != nil {
		t.Fatal(err)
	}
...
// O log da CDN alimenta as métricas, e nenhuma linha pode conter token.
func TestMetricasDaCDNNaoAcusamVazamento(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("métricas da CDN: %v", err)
	}
	defer resp.Body.Close()
	body, _ := io.ReadAll(resp.Body)
	for _, linha := range strings.Split(string(body), "\n") {
		if strings.HasPrefix(linha, "cdn_log_token_leaks_total") && !strings.HasSuffix(strings.TrimSpace(linha), " 0") {
			t.Fatalf("o log da CDN registrou token: %q", linha)
		}
	}
}
