...
func TestObjetoProtegidoExigeSegredoDaCDN(t *testing.T) {
	srv := newServer()
	...
	if rec := do(srv, http.MethodGet, "/objects/img-1KiB-a.bin", map[string]string{HeaderCDNAuth: "errado"}); rec.Code != http.StatusForbidden {
		t.Fatalf("segredo errado: código = %d, esperava 403", rec.Code)
	}

	rec := do(srv, http.MethodGet, "/objects/img-1KiB-a.bin", map[string]string{HeaderCDNAuth: secret})
...
// A revalidação condicional é o que permite à CDN renovar um objeto sem baixar
...
func TestNomeInvalidoDevolve404(t *testing.T) {
	rec := do(newServer(), http.MethodGet, "/objects/nome%20com%20espaco.bin", map[string]string{HeaderCDNAuth: secret})
...
func TestEchoNaoDevolveOSegredo(t *testing.T) {
	...
	if got[HeaderCDNAuth] != "(omitido)" {
		t.Fatalf("o endpoint de diagnóstico devolveu o segredo: %q", got[HeaderCDNAuth])
	}
...
func TestContadoresSeparamCorpoDeRevalidacao(t *testing.T) {
	srv := newServer()
	for range 3 {
		do(srv, http.MethodGet, "/objects/img-1KiB-a.bin", map[string]string{HeaderCDNAuth: secret})
	}
...
func TestModoDeFalhaELatencia(t *testing.T) {
	...
	if got := do(srv, http.MethodGet, "/objects/img-1KiB-a.bin", map[string]string{HeaderCDNAuth: secret}); got.Code != http.StatusServiceUnavailable {
		t.Fatalf("código = %d, esperava 503", got.Code)
	}

	rec = httptest.NewRecorder()
	admin.ServeHTTP(rec, httptest.NewRequest(http.MethodPut, "/admin/fail?status=0", nil))
	if got := do(srv, http.MethodGet, "/objects/img-1KiB-a.bin", map[string]string{HeaderCDNAuth: secret}); got.Code != http.StatusOK {
