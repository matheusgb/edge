...
`evidence/`), então os passos abaixo descrevem como investigar a partir do que
a plataforma realmente expõe, não um número medido de "quantas conexões até
esgotar".
...
4. Checar se a borda está de fato reaproveitando conexões com a origem
   ...
   borda em `internal/edgesrv/edgesrv.go` usa o transport padrão da biblioteca,
   sem um pool dedicado, e um ajuste aqui é a correção mais provável se o
   sintoma for causado por reabertura excessiva de conexões da borda para a
   origem).
...
3. Se for apenas volume genuíno de tráfego, seguir o runbook de latência alta:
   o sintoma de conexões é frequentemente o mesmo problema de capacidade visto
   de outro ângulo.
