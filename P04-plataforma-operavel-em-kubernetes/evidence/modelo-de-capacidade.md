...
réplica da origem: o HPA reagiu de forma proativa antes que qualquer réplica
individual ficasse sobrecarregada, então não existe uma medição direta de
"quantas requisições por segundo uma única réplica aguenta até degradar".
...
origem é `100m` (`infra/terraform/edge-platform/variables.tf`), então, a 100% de
uso desse request, uma réplica sustentaria:
...
(`hpa_target_cpu_utilization_percent = 50`), que é o mesmo ponto em que ele
...
Banda necessária estimada: ~3,28 GB/s no total, ~2,3 MB/s por réplica com 1.422
réplicas, dentro de qualquer limite razoável por réplica: não é o fator
limitante deste cenário.
...
- **"3 zonas" é uma analogia com os 3 nós do cluster kind local** (1
  ...
  `infra/terraform/edge-platform/deployment.tf` distribui pods entre esses nós,
  mas um único host físico continua sendo um ponto único de falha real.
- **1.422 réplicas não foram criadas nem testadas.** O laboratório não prova
  ...
  nessa escala, o que é uma suposição forte que uma medição real de produção
