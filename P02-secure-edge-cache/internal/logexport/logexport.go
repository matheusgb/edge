...
type Collector struct {
	...
	// Responses conta cada resposta da borda por status de cache e código HTTP.
	// É daqui que sai o hit ratio, a prova principal do projeto.
	Responses *prometheus.CounterVec

	// Bytes soma o que a borda entregou, separado por status de cache: mostra
	// quanto tráfego o cache poupou da origem em volume, não só em contagem.
	Bytes *prometheus.CounterVec

	// UpstreamSeconds mede o tempo na origem visto pela borda. Comparado com o
...
func NewCollector() *Collector {
	...
	c.Responses = prometheus.NewCounterVec(prometheus.CounterOpts{
		Name: "edge_responses_total",
		Help: "Respostas da borda, por status de cache e código HTTP.",
	}, []string{"cache", "code"})

	c.Bytes = prometheus.NewCounterVec(prometheus.CounterOpts{
		Name: "edge_response_bytes_total",
		Help: "Bytes entregues pela borda, por status de cache.",
	}, []string{"cache"})

	c.UpstreamSeconds = prometheus.NewHistogram(prometheus.HistogramOpts{
		Name:    "edge_upstream_duration_seconds",
		Help:    "Tempo gasto na origem, quando a borda precisou consultá-la.",
		Buckets: prometheus.ExponentialBuckets(0.001, 2, 14),
	})

	c.RequestSeconds = prometheus.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "edge_request_duration_seconds",
		Help:    "Tempo total da requisição visto pela borda, por status de cache.",
		Buckets: prometheus.ExponentialBuckets(0.001, 2, 14),
	}, []string{"cache"})

	c.Malformed = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "edge_log_malformed_lines_total",
	...
	c.TokenLeaks = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "edge_log_token_leaks_total",
