// Comando loadgen mede o hit ratio da borda e o alívio da origem.
//
...
import (
	...
	"github.com/matheusgb/edge-lab/p02-secure-edge-cache/internal/edgeclient"
	"github.com/matheusgb/edge-lab/p02-secure-edge-cache/internal/evidence"
	"github.com/matheusgb/edge-lab/p02-secure-edge-cache/internal/promscrape"
)
...
func main() {
	edgeURL := flag.String("edge", "http://127.0.0.1:8080", "URL da borda")
	tokendURL := flag.String("tokend", "http://127.0.0.1:8082", "URL do serviço de token")
	edgeMetrics := flag.String("edge-metrics", "http://127.0.0.1:8093/metrics", "métricas derivadas do log da borda")
	originAdmin := flag.String("origin-admin", "http://127.0.0.1:8090", "porta administrativa da origem")
	prefix := flag.String("prefix", "/objects/", "prefixo do caminho na borda")
	...
	client := edgeclient.New(*edgeURL, *tokendURL, *rate)
	...
	if *originMaxAge > 0 {
		if err := edgeclient.SetOriginMaxAge(ctx, *originAdmin, *originMaxAge); err != nil {
	...
	// medido é o da borda, não o do serviço de token. O custo de validar continua
	// dentro da medição, porque a borda consulta o validador a cada requisição.
	...
	edgeBefore, err := promscrape.Fetch(ctx, *edgeMetrics)
	if err != nil {
		fmt.Fprintln(os.Stderr, "erro lendo métricas da borda:", err)
		os.Exit(1)
	}
	originBefore, err := edgeclient.FetchCounters(ctx, *originAdmin)
	if err != nil {
		fmt.Fprintln(os.Stderr, "erro lendo contadores da origem:", err)
		os.Exit(1)
	}

	fmt.Printf("medindo %s a %d req/s em %s%s...\n", *duration, *rate, *edgeURL, *prefix)
	...
	// O exporter lê o log da borda com um pequeno atraso. Sem esta pausa, as
	...
	edgeAfter, err := promscrape.Fetch(ctx, *edgeMetrics)
	if err != nil {
		fmt.Fprintln(os.Stderr, "erro lendo métricas da borda:", err)
		os.Exit(1)
	}
	originAfter, err := edgeclient.FetchCounters(ctx, *originAdmin)
	if err != nil {
		fmt.Fprintln(os.Stderr, "erro lendo contadores da origem:", err)
		os.Exit(1)
	}

	cacheStatus := promscrape.DeltaByLabel(edgeBefore, edgeAfter, "edge_responses_total", "cache")
	result := Result{
		Cenario:          *scenario,
		Prefixo:          *prefix,
		TaxaAlvo:         *rate,
	...
	doc.Notes = "Cliente, borda e origem dividem a mesma máquina e disputam CPU. " +
...
func renderSummary(r Result) string {
	...
	fmt.Fprintf(&b, "| **Hit ratio na borda** | **%.2f%%** |\n", r.HitRatio*100)
...
func buildTargets(ctx context.Context, client *edgeclient.Client, prefix, size, tag string, count int, ttl time.Duration) ([]vegeta.Target, error) {
...
// hitRatio considera acerto tudo que a borda respondeu sem pedir o corpo à
...
func hitRatio(status map[string]float64) float64 {
	...
	for name, count := range status {
		if name == "none" {
			// Respostas geradas pela própria borda (403, 429, 405) não passaram
