...
//
// Ele é o serviço que a borda consulta a cada requisição, então precisa ser
...
func main() {
	...
	keys, err := signer.ParseKeyset(os.Getenv("EDGE_ACTIVE_KEY"), os.Getenv("EDGE_SIGNING_KEYS"))
	if err != nil {
		logger.Error("configuração de chaves inválida", "err", err)
		logger.Error("defina EDGE_SIGNING_KEYS=\"k1:...,k2:...\" e EDGE_ACTIVE_KEY=k2")
		os.Exit(1)
	}
	...
	public := &http.Server{
		...
		// Timeouts curtos: este serviço está no caminho quente da borda. Uma
