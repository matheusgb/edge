// Comando outage mede o que a borda faz quando a origem cai.
//
...
//  4. depois do vencimento, várias requisições são feitas: a borda precisa
...
// a queda é o ANTIGO, e que a borda não transformou uma origem fora do ar em uma
...
func main() {
	...
	client := edgeclient.New(*edgeURL, *tokendURL, 8)
	...
	if err := edgeclient.SetOriginMaxAge(ctx, *originAdmin, *shortTTL); err != nil {
	...
	if err := edgeclient.SetOriginMaxAge(ctx, *originAdmin, *normalTTL); err != nil {
		falhar("erro restaurando o TTL da origem", err)
	}
	if err := edgeclient.SetOriginFailure(ctx, *originAdmin, *failStatus, 0); err != nil {
		falhar("erro derrubando a origem", err)
	}
	defer func() {
		if err := edgeclient.SetOriginFailure(context.Background(), *originAdmin, 0, 0); err != nil {
			fmt.Fprintln(os.Stderr, "aviso: não consegui restaurar a origem:", err)
		}
	}()

	// Espera o vencimento. Enquanto a entrada está fresca, a borda nem consulta a
	// origem, e o experimento estaria medindo um HIT comum.
	time.Sleep(*shortTTL + time.Second)

	antes, err := edgeclient.FetchCounters(ctx, *originAdmin)
	if err != nil {
	...
	depois, err := edgeclient.FetchCounters(ctx, *originAdmin)
	if err != nil {
		falhar("erro lendo contadores da origem", err)
	}
	...
	// revalidação.
	if err := edgeclient.SetOriginFailure(ctx, *originAdmin, 0, 0); err != nil {
		falhar("erro restaurando a origem", err)
	}
	antesDaVolta, err := edgeclient.FetchCounters(ctx, *originAdmin)
	if err != nil {
		falhar("erro lendo contadores da origem", err)
	}
	for i := range 5 {
	...
	depoisDaVolta, err := edgeclient.FetchCounters(ctx, *originAdmin)
	if err != nil {
		falhar("erro lendo contadores da origem", err)
	}
...
func requisitar(client *edgeclient.Client, url, nome string) Passo {
