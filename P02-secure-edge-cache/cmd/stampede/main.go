...
// instante, cem clientes pedem por ele. Sem coordenação, a borda abre cem
...
// em alta.
//
// O experimento roda o mesmo disparo em dois caminhos da borda, um com
...
// Variante é um disparo contra um caminho da borda.
...
func main() {
	...
	client := edgeclient.New(*edgeURL, *tokendURL, *clients)
	...
	if err := edgeclient.SetOriginLatency(ctx, *originAdmin, *originLatency); err != nil {
		fmt.Fprintln(os.Stderr, "erro configurando latência da origem:", err)
		os.Exit(1)
	}
	defer func() {
		if err := edgeclient.SetOriginLatency(context.Background(), *originAdmin, 0); err != nil {
...
func dispararVariante(ctx context.Context, client *edgeclient.Client, originAdmin, nome, caminho string, lock bool, clients int, timeout time.Duration) (Variante, error) {
	out := Variante{
	...
	before, err := edgeclient.FetchCounters(ctx, originAdmin)
	if err != nil {
		return out, err
	}
	...
	after, err := edgeclient.FetchCounters(ctx, originAdmin)
	if err != nil {
		return out, err
	}
