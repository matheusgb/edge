# P02: borda segura com cache

> **A pergunta deste lab:** como uma borda pode guardar UMA cópia de um arquivo e
...
Uma borda (ou CDN) é um servidor que fica entre o usuário e o sistema que produz
o conteúdo. Ela guarda cópias do que já foi pedido e responde de novo sem
incomodar quem produziu. É assim que um vídeo assistido por um milhão de pessoas
não vira um milhão de leituras no servidor original.
...
tem um "arquivo" diferente aos olhos da borda: mil cópias guardadas, mil idas à
...
+------------------------------------------+
   ...
   v
origem (Go), alcançável só pela borda
```
...
de uma borda, e a mais fácil de errar.
...
**Por que assinatura e não senha?** Porque a borda não precisa consultar banco
...
assim ([test/edge_integration_test.go](test/edge_integration_test.go),
...
token de leitura autorizaria qualquer coisa que a borda deixasse passar. Por isso
...
do Go. É **exigir que as duas cheguem ao mesmo resultado**: a borda manda também
...
cem clientes pedem por ele. Sem coordenação, a borda abre cem conexões com a
...
Quando a origem está fora do ar, a borda tem duas opções: propagar o erro ou
...
borda então pergunta à origem: "ainda vale este ETag?". Se a resposta for **304
...
Cliente, borda, validador e origem rodam todos aqui dentro, disputando a mesma
...
| ------------ | ------------------ | -----------------: | ------------: | -------: | -------: |
| **com lock** | ligado             |              **1** |           1,0 | 621,7 ms | 694,0 ms |
| **sem lock** | desligado          |            **100** |         100,0 | 319,9 ms | 375,9 ms |
...
| Hit ratio na borda             |     98,00% |     98,00% |
| **Chamadas à origem**          |    **200** |    **816** |
| das quais, corpos completos    |        200 |        200 |
| das quais, revalidações (304)  |          0 |        616 |
| **Bytes servidos pela origem** | **12,5 MiB** | **12,5 MiB** |
| Alívio da origem               |     98,00% |     91,84% |
| p50 / p99 no cliente           | 0,34 / 0,65 ms | 0,35 / 0,69 ms |
...
multiplicou as chamadas por quatro e não transferiu um byte a mais, porque as 616
...
**Tentativas por pedido = 1,00 é o número que importa aqui.** Uma borda mal
...
Isso também expõe um limite honesto: a borda não amplifica, mas também **não para
...
rota de diagnóstico da borda estava entregando a query string inteira à origem, e
...
Para uma borda que atende 50 mil requisições por segundo, a validação de token
...
isso que a configuração do Nginx é um template com `${EDGE_ORIGIN_SECRET}`,
...
| `logexporter` | Go: transforma o log da borda em métricas Prometheus    | `127.0.0.1:8093`      |

**A origem não tem porta publicada de propósito.** Ela só existe dentro da rede do
Compose, alcançável pela borda. Se estivesse publicada, um experimento poderia
...
# o link devolvido é usado na borda
...
```bash
curl -s localhost:8093/metrics | grep edge_          # hit ratio, tempo na origem, vazamentos
...
O `edge_log_token_leaks_total` merece atenção: ele conta linhas do log da borda
...
(signed URL), token de borda, chave de cache customizada, request collapsing,
...
borda: limite de método, limite de corpo, limite de taxa, remoção de headers não
...
**Uma região só, sem geografia.** Isto simula uma borda, não uma CDN global. Não
há presença geográfica, roteamento por proximidade, nem a rede privada entre
borda e origem que um provedor real opera.

**Não há Range nem entrega parcial.** A borda apaga o header `Range`. Cachear
...
borda aguenta X requisições por segundo".
...
cmd/
  ...
  logexporter/  segue o log da borda e expõe métricas Prometheus
...
internal/
  ...
  edgeclient/   o cliente que os experimentos usam
  evidence/     o pacote de evidência de cada execução
deploy/nginx/   a borda: nginx.conf e o template com os controles
...
ter a própria cópia, e a borda vira um proxy caro.

A saída é separar as duas perguntas. **"Que arquivo é este?"** define o que a
borda guarda, e a resposta não pode depender de quem está pedindo. **"Você pode
...
caminho literal e `proxy_pass` com variável. Segurança de borda não se lê na
